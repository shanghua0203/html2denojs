...
function submitQuestion() {
    ...
    if (location.protocol != 'file:')
        chat(questionNode.value).then((response)=>{
            ...
            document.querySelector(`#status_${chatId}`).innerHTML = `<label onclick="viewChat('${chatId}')">檢視</label>`
        }).catch((error)=>{
    ...
    let statusCell = row.insertCell(2); statusCell.innerHTML = `<div id="status_${chatId}">等待中</div>`
